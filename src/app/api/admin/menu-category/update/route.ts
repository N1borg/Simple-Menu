...
export async function POST(req: NextRequest) {
  ...
  const { id, name, display_style, display_order, is_available, slug: bodySlug } = await req.json()
  ...
  const { error } = await supabase
    .from('categories')
    .update({ name: safeName, display_style: safeStyle, display_order: safeOrder, is_available: safeAvailable })
    .eq('id', id)
  ...
  auditLog({ action: 'category_update', ip, details: { id, name, display_style, display_order, is_available } })
