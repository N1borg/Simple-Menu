...
export async function GET(req: NextRequest) {
  ...
  try {
    // Find the establishment by slug and get basket_enabled status
    // Note: If basket_enabled column doesn't exist, it will be undefined
    const { data: establishment, error: fetchError } = await supabase
      .from('establishments')
      .select('id, slug, basket_enabled')
      .eq('slug', slug)
      .single()

    if (fetchError || !establishment) {
      // If error is due to missing column, return default value
      if (fetchError?.message.includes('basket_enabled') && fetchError?.message.includes('column')) {
        return NextResponse.json({ 
          basketEnabled: true, // Default to enabled
          migration_needed: true
        })
      }

      await auditLog({
        action: 'get_basket_enabled_failed',
        user: slug,
        details: { reason: 'Establishment not found', error: fetchError?.message }
      })
      return NextResponse.json(
    ...
    return NextResponse.json({ 
      basketEnabled: establishment.basket_enabled ?? true // Default to true if not set
    })
    ...
    console.error('Error in basket get API:', error)
    return NextResponse.json(
      { error: 'Erreur interne du serveur' },
      { status: 500 }
    )
