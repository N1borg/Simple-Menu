  ...
  return (
    <div className={`text-sm ${className}`}>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-2">
              {establishmentInfo.address && (
                <div className="flex items-start gap-2">
                  <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: color || '#3a4fff' }} />
                  <span>{establishmentInfo.address}</span>
                </div>
              )}
              {establishmentInfo.phone && (
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4 flex-shrink-0" style={{ color: color || '#3a4fff' }} />
                  <a href={`tel:${establishmentInfo.phone}`} className="hover:underline">
                    {establishmentInfo.phone}
                  </a>
                </div>
              )}
              {establishmentInfo.email && (
                <div className="flex items-center gap-2">
                  <Mail className="w-4 h-4 flex-shrink-0" style={{ color: color || '#3a4fff' }} />
                  <a href={`mailto:${establishmentInfo.email}`} className="hover:underline">
                    {establishmentInfo.email}
                  </a>
                </div>
              )}
              ...
              <div className="mt-4 pt-3 border-t" style={{ borderColor: (color || '#3a4fff') + '20' }}>
                <div className="flex gap-4">
                  {establishmentInfo.facebook_url && (
                    ...
                    >
                      <FacebookIcon className="w-6 h-6" style={{ color: color || '#3a4fff' }} />
                    </a>
                  )}
                  {establishmentInfo.instagram_url && (
                    <a
                      ...
                      className="hover:opacity-80 transition-opacity"
                    >
                      <InstagramIcon className="w-6 h-6" style={{ color: color || '#3a4fff' }} />
                    </a>
                  )}
                </div>
              </div>
        ...
        {establishmentInfo.opening_hours && establishmentInfo.opening_hours.length > 0 && (
          <div>
            <h4 className="font-semibold mb-3 flex items-center gap-2" style={{ color: color || '#3a4fff' }}>
              <Clock className="w-4 h-4" style={{ color: color || '#3a4fff' }} />
              Horaires
            </h4>
