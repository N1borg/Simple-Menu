...
export default function Basket({ establishmentColor, isAdminView = false, basketEnabled = true, adminBannerPresent = false }: BasketProps) {
  ...
  return (
    <>
      <div className={`w-full sticky z-20 transition-all duration-300 ${adminBannerPresent ? 'top-[41px]' : 'top-0'}`}> 
        {/* Basket header bar */}
        <div 
      ...
      {/* Overlay basket dropdown */}
      {isExpanded && (
        <div
          className="fixed inset-0 z-50 flex flex-col items-center justify-start"
          style={{ top: adminBannerPresent ? 41 : 0 }}
        >
          {/* Optional: semi-transparent background for focus */}
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setIsExpanded(false)} />
          <div className="relative w-full max-w-2xl mx-auto mt-0 bg-white rounded-b-2xl shadow-2xl overflow-hidden animate-fadeInUp" style={{ zIndex: 51 }}>
            <div className="px-4 p-4 h-full flex flex-col relative">
